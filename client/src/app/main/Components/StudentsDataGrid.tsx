...
import { Student, StudentsProps } from "@/app/types/types";
import {
  Table,
  TableHeader,
  TableColumn,
...
const INITIAL_VISIBLE_COLUMNS = ["fullName", "email", "level", "state", "actions"];

export default function StudentsDataGrid({props} : StudentsProps) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (statusFilter !== "all" && Array.from(statusFilter).length !== students.length) {
      filteredStudents = filteredStudents.filter((student) =>
        Array.from(statusFilter).includes(student.state),
      );
    }
  ...
  const renderCell = React.useCallback((student: Student, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      case "state":
        return (
          <Chip className="capitalize" color={statusColorMap[student.state]} size="sm" variant="flat">
            {cellValue}
          </Chip>
        );
      case "actions":
  ...
  const [inputs , setInputs] = React.useState<Student>(initialState);
    ...
    <>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} >
        <ModalContent className="bg-gray-950">
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Add a student</ModalHeader>
              <ModalBody>
